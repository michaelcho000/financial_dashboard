    ...
    <>
      ...
      {isConfirmingDelete && (
        <ConfirmationModal
          ...
          title="계정 삭제 확인"
          message={`'${account.name}' 계정을 삭제하시겠어요? 이 작업은 되돌릴 수 없습니다.`}
...
const GroupEditor: React.FC<GroupEditorProps> = ({
  ...
  const handleDeleteGroup = () => {
    if (accounts.length > 0) {
      alert('그룹에 속한 계정이 있어 삭제할 수 없습니다. 먼저 다른 그룹으로 이동하거나 계정을 삭제해주세요.');
      setIsConfirmingDelete(false);
      return;
    }
...
const AccountManagementPage: React.FC = () => {
  const { variable, currentMonths, setCurrentMonths, commitDraft, resetDraft, versions } = useFinancials();
  ...
  }, [accounts, accountGroups, buildDraftAccounts, buildDraftGroups, isDirty, versions.draft]);

  const markDirty = useCallback(() => {
    setIsDirty(true);
  }, []);

  const updateDraftAccounts = useCallback((category: AccountCategory, updater: (accounts: Account[]) => Account[]) => {
  ...
  const handleSave = useCallback(() => {
    ...
    setIsDirty(false);
    commitDraft(applied);
    notifySave('계정 구조를 저장했습니다.');
  }, [draftAccounts, draftGroups, saveStructure, commitDraft, notifySave]);
  ...
  }, [resetDraft, buildDraftAccounts, buildDraftGroups, notifyCancel]);

  return (
    <>
      <Header
        title="계정 관리"
        description="손익계산서에서 사용할 계정과목 구조를 설정하세요."
      ...
      <div className="mt-6 flex justify-end gap-3">
        <button
          ...
          disabled={!isDirty}
          className={`px-4 py-2 rounded-md text-base font-semibold ${isDirty ? 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-100' : 'bg-gray-200 text-gray-500 cursor-not-allowed'}`}
        >
          변경 취소
        </button>
        <button

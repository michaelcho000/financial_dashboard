...
### 2.4 POST `/baselines/{id}/lock`
- 설명: 기준월을 잠금 처리.
- 응답: `200 OK`, 실패 시 `409`.

### 2.5 POST `/baselines/{id}/unlock`
- 설명: 잠금 해제 (admin 전용).
- 응답: `200 OK`, 권한 부족 시 `403`.

## 3. 고정비 선택
### 3.1 GET `/baselines/{id}/fixed-costs`
...
### 3.2 PUT `/baselines/{id}/fixed-costs`
...
### 4.2 PUT `/baselines/{id}/consumables`
...
## 5. 시술 데이터
### 5.1 GET `/baselines/{id}/procedures`
...
### 5.2 POST `/baselines/{id}/procedures`
- 설명: 새 시술(또는 변형) 추가.
- 본문: 위 구조 참고. `procedureId` 없으면 새 시술 생성.
- 응답: `201 Created`

### 5.3 PUT `/baselines/{id}/procedures/{variantId}`
- 설명: 시술 변형 업데이트.
- 응답: `200 OK`

### 5.4 DELETE `/baselines/{id}/procedures/{variantId}`
- 설명: 변형 삭제.
- 응답: `204 No Content`
- 에러: 계산 결과에 이미 포함된 경우 `409`.

## 6. 계산 & 결과
### 6.1 POST `/baselines/{id}/recalculate`
- 설명: 원가 계산 실행 (항상 비동기).
- 응답: `202 Accepted`
```json
{
...
### 6.4 GET `/baselines/{id}/export`
- 설명: CSV/Excel 다운로드.
- 쿼리: `format=csv|xlsx`
- 응답: `200 OK` + 파일 스트림.

## 7. 감사 로그 & 메타
### 7.1 GET `/baselines/{id}/changes`
...
---
- 향후 확장: `GET /baselines/{id}/summary`(대시보드용), 실시간 계산 상태 업데이트를 위한 WebSocket/Server-Sent Events 도입 검토.
- 문서화: README/AGENTS.md 에 API 사용법을 요약하고, Supabase 연동 시 Endpoint URL 및 인증 흐름을 추가 기재한다.

## 결정 사항 업데이트
- `POST /baselines/{id}/recalculate` 는 항상 비동기 처리로 고정한다.
- 응답은 `202 Accepted` 와 함께 `{ "status": "QUEUED", "jobId": "uuid", "queuedAt": "ISO8601" }` 형태를 반환한다.
- 클라이언트는 `jobId` 를 활용해 `/baselines/{id}/results` 또는 `/baselines/{id}/insights` 를 재조회하거나, 향후 도입할 결과 폴링/푸시 채널을 통해 계산 완료를 감지한다.
